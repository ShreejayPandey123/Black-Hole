...
vec3 shade_disk(float r, float ph, float E, float L, float a, float rin, float t) {
    ...
    if (ut2 > 1e-6) {
        ...
        vec3 base_col = blackbody_rgb(T_obs) * intensity * 1.6 * u_opt_density;
        col = base_col * vec3(1.3, 0.45, 0.08);
    }
    return col;
}
...
void main() {
    vec2 wobble_offset = vec2(
        sin(u_time * 65.0) * 0.0018,
        cos(u_time * 58.0) * 0.0018
    ) * u_wobble;
    vec2 uv = (gl_FragCoord.xy + 0.5 - 0.5 * u_resolution) / u_resolution.y + wobble_offset;
    ...
    if (u_lensing < 0.5) {
            ...
            if (t_int > 0.0) {
                ...
                if (r_int >= r_isco && r_int <= u_disk_r_out) {
                    ...
                    if (ut2 > 1e-6) {
                        ...
                        float tex = 0.22 + 1.3 * pow(fbm(pco * 1.3), 2.0);
                        float intensity = pow(gfac, 4.0) * pow(T_em / u_disk_temp, 4.0) * tex;
                        vec3 base_col = blackbody_rgb(gfac * T_em) * intensity * 1.6 * u_opt_density;
                        // Tint to fiery orange/red to match reference image
                        col = base_col * vec3(1.3, 0.45, 0.08);
                    }
        ...
        for (int step = 0; step < u_quality_steps; step++) {
            // Take much larger steps when far away (in flat space) to reach the black hole without running out of steps.
            float hs = 0.16 * max(y.r * 0.25, 0.05);
            hs *= clamp((y.r - rstop) * 2.0, 0.05, 1.0);
            hs *= clamp(abs(sin(y.th)) * 3.5, 0.15, 1.0);
            
            yn = rk4_step(y, hs, E, L, a);
            
            // Volumetric Jet march — turbulent, non-uniform
            if (u_jets > 0.5) {
                ...
                if (abs(rz) > rstop) {
                    ...
                    // ---- Turbulent jet width ----
                    // Base width grows slowly with altitude
                    float jet_r_base = 0.50 + 0.038 * abs(rz);
                    // Low-frequency lateral wobble (sinusoidal along z)
                    float wobble_freq = 0.18;
                    float wobble_amp  = 0.12 * (1.0 - exp(-abs(rz)*0.06)); // grows from base
                    float wobble_speed = 0.7 + u_wobble * 4.5;
                    float wobble_mult  = 1.0 + u_wobble * 3.5;
                    float wobble = sin(abs(rz) * wobble_freq - u_time * wobble_speed) * wobble_amp * wobble_mult;
                    // High-frequency FBM width jitter
                    vec2 width_uv = vec2(abs(rz) * 0.14 - u_time * 0.4, y.ph * 0.5);
                    float width_noise = fbm(width_uv) * 0.18 - 0.09;
                    float jet_r = jet_r_base + wobble + width_noise;

                    // ---- Lateral axis offset (makes jet wobble side to side) ----
                    // Offset the axis test so the jet centreline itself moves
                    vec2 axis_uv = vec2(abs(rz) * 0.10 - u_time * (0.55 + u_wobble * 2.0), 3.3);
                    float axis_wobble = (fbm(axis_uv) - 0.5) * 0.20 * (1.0 - exp(-abs(rz)*0.05)) * wobble_mult;
                    float effective_dist = abs(dist_axis - axis_wobble);

                    if (effective_dist < jet_r) {
                        float z_decay = 1.0 / (1.0 + 0.015 * abs(rz));
                        float radial_decay = exp(-3.0 * pow(effective_dist / jet_r, 2.0));

                        // ---- Brightness fluctuation along the jet ----
                        float flow_speed = 3.5;
                        vec2 noise_uv = vec2(y.ph * 0.35, abs(rz) * 0.22 - u_time * flow_speed);
                        float brightness_noise = 0.35 + 1.1 * fbm(noise_uv); // wider range
                        // Additional fine-scale flicker
                        vec2 fine_uv = vec2(abs(rz) * 0.55 - u_time * 5.5, y.ph * 1.2);
                        brightness_noise += (fbm(fine_uv) - 0.5) * 0.3;
                        brightness_noise = clamp(brightness_noise, 0.1, 1.8);

                        float density = z_decay * radial_decay * brightness_noise * u_jet_power * 0.42 * hs;
                        ...
                        float u_norm = effective_dist / jet_r;
            ...
            if (c0 * c1 < 0.0) {
                ...
                if (rx >= r_isco && rx <= u_disk_r_out) {
                    ...
                    // ---- Photon ring: thin bright bluish-white ring ----
                    // The photon sphere sits at r = 3M (Schwarzschild) but in
                    // Kerr it shifts; we use a narrow band around r_isco*1.05
                    float r_photon = r_isco * 1.05;
                    float ring_width = 0.08;          // very thin
                    float ring_dist = abs(rx - r_photon);
                    float ring_mask = exp(-pow(ring_dist / ring_width, 2.0) * 6.0);
                    // Slightly blue-shifted white
                    vec3 ring_col = vec3(0.85, 0.93, 1.0) * 4.5;
                    col += ring_col * ring_mask;
